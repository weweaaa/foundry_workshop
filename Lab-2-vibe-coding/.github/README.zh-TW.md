...
- `.github/agents/*.agent.md`（本 workshop 目前檔名為 `*.chatmode.md`）— 自訂 agent mode（頂部下拉切換）
...
| 檔案 | 類型 | 觸發方式 |
|------|------|----------|
| `agents/maf-agent.chatmode.md` | agent mode | Copilot Chat 頂部下拉選擇 "maf-agent" |
...
VS Code 1.95+ 預設啟用 agents/prompts/instructions。如果沒看到，執行：
...
- 將 `agents/maf-agent.chatmode.md` 頂部的 "Default scenario" 換成你的業務。
