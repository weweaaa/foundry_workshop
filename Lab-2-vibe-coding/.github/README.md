...
- `.github/agents/*.agent.md`（本 workshop 当前文件名为 `*.chatmode.md`）— 自定义 agent mode（顶部下拉切换）
- `.github/instructions/*.instructions.md` — 按 `applyTo` glob 注入到匹配文件的 Copilot 上下文
- `.github/prompts/*.prompt.md` — `/<slug>` 斜杠命令

学员在 Lab 0 cd 到 `Lab-2-vibe-coding` 并 `code .`，本目录的所有文件即自动生效。

## 提供的 skill

| 文件 | 类型 | 触发方式 |
|------|------|---------|
| `agents/maf-agent.chatmode.md` | agent mode | Copilot Chat 顶部下拉选择 "maf-agent" |
...
VS Code 1.95+ 默认启用 agents/prompts/instructions。如果没看到,运行:
...
- agents/maf-agent.chatmode.md 顶部"Default scenario"换成你的业务
